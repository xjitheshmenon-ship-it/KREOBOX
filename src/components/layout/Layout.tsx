  ...
  return (
    <div className="flex h-screen bg-canvas overflow-hidden">
      <nav className="w-56 bg-sidebar flex flex-col shrink-0">
        {/* Logo */}
        <Link to="/contractor" className="px-5 py-5 flex items-center gap-2.5 hover:opacity-80 transition-opacity">
        ...
        <div className="px-3 mt-4">
          <p className="text-[10px] text-white/30 uppercase tracking-widest font-medium px-2 mb-1">Merchandise</p>
          <NavItem to="/customer"   label="Home Catalog"  active={pathname === '/customer'} />
          <NavItem to="/office"     label="Office Catalog" active={pathname === '/office'} />
        </div>

        {/* Platform */}
        <div className="px-3 mt-4">
          <p className="text-[10px] text-white/30 uppercase tracking-widest font-medium px-2 mb-1">Platform</p>
        ...
        <div className="mt-auto px-4 py-4">
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0">
              <p className="text-xs font-medium text-white truncate">Contractor</p>
              <p className="text-[10px] text-white/40">DesignOS Pro</p>
            </div>
          </div>
        </div>
      </nav>
...
function NavItem({ to, label, active, dot }: { to: string; label: string; active: boolean; dot?: boolean }) {
  return (
    ...
    >
      {dot && (
        <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${active ? 'bg-accent' : 'bg-white/30'}`} />
      )}
